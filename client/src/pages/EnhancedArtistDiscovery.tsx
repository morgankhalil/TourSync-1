...
export default function EnhancedArtistDiscovery() {
  ...
  const [previousVenueId, setPreviousVenueId] = useState<number | null>(venueId);
  
  // Reset when venue changes 
  useEffect(() => {
    // Check if venue has actually changed
    if (previousVenueId !== venueId) {
      console.log(`Venue changed from ${previousVenueId} to ${venueId}`);
      console.log("New venue object:", activeVenue);
      
      // Clear results
      setSearchResults([]);
      setSelectedArtist(null);
      setErrorMessage(null);
      setPreviousVenueId(venueId);
